...
export const adminUserUnfollow = async (req, res) => {
  try {
    const user = await User.findByIdAndUpdate(
      req.body.user_id,
      {
        $pull: { following: req.body.body_id },
      },
      { new: true }
    );
    res.json(user);
...
export const adminRemoveFollower = async (req, res, next) => {
  try {
    const user = await User.findByIdAndUpdate(req.body.body_id, {
